...
;(function(window) {
		var DB = {
				...
				updateTaskDB: function(data, callback) { // 更新一条记录 数据库-任务
						...
						DB.executeSql(DB.taskNameDB, 'UPDATE ' + DB.taskNameTable + ' SET progress="' + data.progress + '",state="' + data.state + '",createDate="' + data.createDate + '" WHERE taskId="' + data.taskId + '" and isLog="0"', function(ret, err) {
